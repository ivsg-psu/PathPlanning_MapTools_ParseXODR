% script_test_fcn_ParseXODR_fillBlankFieldStructure
% Exercises the function: fcn_ParseXODR_fillBlankFieldStructure
...
% Check that the fields were created
assert(isfield(output_struct,'a'));
assert(isfield(output_struct,'b'));
assert(isfield(output_struct,'temp'));

%% BASIC test - fast mode

output_struct = fcn_ParseXODR_fillBlankFieldStructure({'a','b','temp'},-1);
