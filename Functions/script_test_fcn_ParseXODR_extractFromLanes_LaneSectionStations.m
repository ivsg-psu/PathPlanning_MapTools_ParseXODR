%% script_test_fcn_ParseXODR_extractFromLanes_LaneSectionStations
% Script to extract LaneGeometry in an XODR file.
% Tests function fcn_ParseXODR_extractFromLanes_LaneSectionStations
%
...
laneSectionStations = fcn_ParseXODR_extractFromLanes_LaneSectionStations(current_lanes, road_length, fig_num);

% Check lengths - this example has 5 lane offset segments
assert(length(laneSectionStations(:,1))==5);
assert(length(laneSectionStations(1,:))==2);

% Check values - segments should start at 0 and end at road length
assert(isequal(round(laneSectionStations(1,1),4),round(0,4)));
assert(isequal(round(laneSectionStations(end,2),4),round(road_length,4)));
...
assert(length(laneSectionStations(:,1))==3);
assert(length(laneSectionStations(1,:))==2);
...
assert(length(laneSectionStations(:,1))==1);
assert(length(laneSectionStations(1,:))==2);
