% script_test_fcn_ParseXODR_checkXODRSegments
% Script to test XODR structure compliance
% Tests function fcn_ParseXODR_checkXODRSegments
%
% Revision history:
...
% Check the structure
[ODRStruct, flag_warnings_were_found] = fcn_ParseXODR_checkXODRSegments(ODRStruct);
...
% Check the structure
[ODRStruct, flag_warnings_were_found] = fcn_ParseXODR_checkXODRSegments(ODRStruct, fig_num);
...
% Force the geometry elements to have a bad station sequence
ODRStruct_BAD = ODRStruct;
true_length = str2double(ODRStruct.OpenDRIVE.road{1}.planView.geometry{1}.Attributes.length);
ODRStruct_BAD.OpenDRIVE.road{1}.planView.geometry{1}.Attributes.length = sprintf('%.16e',true_length*0.01);
...
[ODRStruct_fixed, flag_warnings_were_found] = fcn_ParseXODR_checkXODRSegments(ODRStruct_BAD);

% Make sure we get a structure back
assert(isstruct(ODRStruct_fixed));
assert(isequal(flag_warnings_were_found,1));

% Check that the new structure is NOT fixed
[~, flag_warnings_were_found] = fcn_ParseXODR_checkXODRSegments(ODRStruct_fixed);
